...
public class RenderOutOfOrderMachine extends TileEntitySpecialRenderer {
	private final ModelPennyPusher1 model;

	public RenderOutOfOrderMachine() {
	...
	public void renderTileEntityAt(TileEntity te, double x, double y, double z,
			float scale) {
		TileEntityOutOfOrderMachine tile = (TileEntityOutOfOrderMachine) te;
		
    	int direction = te.getWorldObj().getBlockMetadata(te.xCoord, te.yCoord, te.zCoord);
		
    	GL11.glPushMatrix();
	    GL11.glTranslatef((float) x + 0.5F, (float) y + 1.5F, (float) z + 0.5F);
		ResourceLocation textures = (new ResourceLocation(PennyArcade.MODID + ":textures/blocks/model/pennyarcade/OutOfOrder.png")); 
