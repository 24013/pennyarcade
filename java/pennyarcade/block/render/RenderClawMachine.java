...
public class RenderClawMachine extends TileEntitySpecialRenderer {
	private final ModelClawMachine model;

	public RenderClawMachine() {
		this.model = new ModelClawMachine();
	}
	...
	public void renderTileEntityAt(TileEntity te, double x, double y, double z, float scale) {
		TileEntityClawMachine tile = (TileEntityClawMachine) te;
		
    	int direction = te.getWorldObj().getBlockMetadata(te.xCoord, te.yCoord, te.zCoord);
    	
    	GL11.glPushMatrix();
	    GL11.glTranslatef((float) x + 0.5F, (float) y + 1.5F, (float) z + 0.5F);
		ResourceLocation textures = (new ResourceLocation(PennyArcade.MODID + ":textures/blocks/model/pennyarcade/ClawMachine.png")); 
